...
```{r}
library(readxl)
sheet1 <- readxl::read_xlsx("SmallBowelMotilityTable.xlsx") %>% select(Patient:EatB2) %>% mutate(reader= "Investigator1")
sheet2 <- readxl::read_xlsx("SmallBowelMotilityTable.xlsx", sheet=2) %>% select(Patient:EatB2) %>% mutate(reader="Investigator2")

motility <- rbind(sheet1, sheet2)
...
```{r}
motility[,3:10] <- motility[,3:10]+0.001

motlong <- motility %>% pivot_longer(
  cols = Fast1:EatB2,
  names_to = "times",
  values_to = "value"
) %>% pivot_wider(
...
motlonger <- motility %>% pivot_longer(
  cols = Fast1:EatB2,
  names_to = "times",
  values_to = "value"
)
...
motscans <- motlonger %>% 
  mutate(scans = case_when(
  times %in% c("Fast1", "Gum1", "EatA1", "EatB1") ~ "Scan1",
  times %in% c("Fast2", "Gum2", "EatA2", "EatB2") ~ "Scan2"
)) %>% 
  mutate(timing = case_when(
    times %in% c("Fast1", "Fast2") ~ "Fast",
    times %in% c("Gum1", "Gum2") ~ "Gum",
    times %in% c("EatA1", "EatA2") ~ "EatA",
    times %in% c("EatB1", "EatB2") ~ "EatB",
  )) %>% 
  select(-times) %>% 
  pivot_wider(names_from = scans,
              values_from = value)

plot(x=motlong$value[motlong$Position=="LUQ"], y=motlong$value[motlong$Position=="RLQ"], type="p")
```
...
cor(motlong$LUQ, motlong$RLQ)
t.test(motlong$LUQ, motlong$RLQ, paired=T)

plot(motility$Fast1[motility$Position=="LUQ"], motility$Fast1[motility$Position=="RLQ"])
```
...
cor(motscans$Scan1, motscans$Scan2)
t.test(motscans$Scan1, motscans$Scan2, paired=T)

cor(motlong$LUQ, motlong$RLQ)
t.test(motlong$LUQ, motlong$RLQ, paired=TRUE)


#motility %>%
#  select(Patient, c(Fast1, Fast2)) %>%
#  group_by(Patient) %>%
...
```{r}
motres <- pivot_wider(motlonger, names_from = reader, values_from = value)

ggplot(motres,
       aes(x=Investigator1, y=Investigator2,  colour = Position)) +
  geom_point() +
  geom_jitter() +
  geom_smooth(method="lm")

cor(motres$Investigator1, motres$Investigator2)
t.test(motres$Investigator1, motres$Investigator2, paired = T)

ggplot(motlong, 
       aes(y=LUQ, colour=times)) +
  geom_boxplot() +
  geom_boxplot(data=motlong, aes(y=RLQ, colour=times))

ggplot(motlong,
       aes(x=LUQ, y=RLQ, colour = times)) +
  geom_point() +
  geom_jitter() +
  geom_smooth()

motlonger %>% filter(times %in% c("Fast1", "Fast2", "Gum1", "Gum2")) %>%  ggplot(aes(y=value, x=times, colour = times)) + geom_violin()

hypoth1 <- motlonger %>% filter(times %in% c("Fast1", "Fast2", "Gum1", "Gum2", "EatA1", "EatA2", "EatB1", "EatB2")) %>% pivot_wider(names_from = times) %>% mutate(
  Fast=rowMeans(select(., starts_with("Fast"))), 
  Gum=rowMeans(select(., starts_with(("Gum")))),
  EatA=rowMeans(select(., starts_with("EatA"))),
  EatB=rowMeans(select(., starts_with("EatB"))))

t.test(hypoth1$Fast, hypoth1$EatB)
